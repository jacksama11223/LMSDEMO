...
const GeminiStudentPage: React.FC = () => {
    ...
    const handlePersonaChange = (p: Persona) => {
        setActivePersona(p);
        // Optional: Reset chat or add a system message indicating persona change
    };

    const handleSend = useCallback(async (e: React.FormEvent) => {
        e.preventDefault();
        if (!prompt.trim() || orbState === 'thinking') return;
        ...
        try {
            // Pass the persona's specific system prompt
            ...
            // Reset orb to idle after a short delay
    ...
    }, [prompt, orbState, user, db.USERS, activePersona]);
    ...
    // Gamification: Save Insight
    const crystallizeInsight = (text: string) => {
        // In a real app, this would save to a "Notebook" DB
        // For now, visual feedback + fake XP
        alert(`💎 Đã kết tinh tri thức!\n+50 XP vào Kho báu.`);
        // Trigger global XP update here if possible via context
    };
    ...
    return (
        <div className="h-[calc(100vh-100px)] flex flex-col lg:flex-row gap-6 relative overflow-hidden">
            ...
            {/* LEFT PANEL: ORACLE VISUAL & PERSONA SELECTOR */}
            ...
            {/* RIGHT PANEL: CHAT INTERFACE (Holographic Console) */}
                    ...
                    {chatHistory.map((msg, index) => (
                        <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start group'}`}>
                            <div className={`relative max-w-[85%] p-4 rounded-2xl border backdrop-blur-md transition-all duration-300 hover:scale-[1.01]
                                ...
                                <div className="prose prose-invert max-w-none prose-sm leading-relaxed" dangerouslySetInnerHTML={{ __html: msg.parts[0].text.replace(/\n/g, '<br />') }} />
                                
                                {/* Crystallize Button (Only for AI messages) */}
                                {msg.role === 'model' && (
                                    <button 
